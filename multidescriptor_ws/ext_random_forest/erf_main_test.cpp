...
double eval_set(ERF_class* erf, const cv::Mat& test_set, const cv::Mat& test_labels, const cv::Mat& features_per_image, 
	...
	if(eval_per_feature){
		for(int ex = 0; ex < test_set.rows; ex++)
		{
			...
			int prediction;
			erf->predict_with_hist(test_set.row(ex),&prediction);
...
int main ( int argc, char *argv[] ){
	...
	std::string model_params_file = argv[1];//"erf_parameters.yml";
	...
	if(save_model){
		...
		erf->save_model(model_file, model_name, training_set);
	...
	good_classif = 0;
	if(test_trainSet){
		good_classif = eval_set(erf_copy, train_samples, train_labels, train_features_per_image, classes, eval_per_feature);
		std::cout << "Training accuracy: " << good_classif << std::endl;
	}
	if(test_testSet){
		good_classif = eval_set(erf_copy, test_samples, test_labels, test_features_per_image, classes, eval_per_feature);
		std::cout << "Testing accuracy: " << good_classif << std::endl;
	}
	//<[DEBUGGING]
