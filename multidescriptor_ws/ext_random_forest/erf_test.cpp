...
*/
	...
	std::string model_file= "erf_model.yml";
	std::string model_name = "erf_model";
	erf->save_model(model_file, model_name);
	std::cout << "Model saved" << std::endl;
	
	//+++ TEST LOADING THE MODEL +++//
	std::cout << "Test loading the model" << std::endl;
	ERF_class* erf_copy = new ERF_class();
	erf_copy->load_model(model_file, model_name);
	std::cout << "Model loaded" << std::endl;
	
	//+++ TEST SAVING THE MODEL +++//
	std::cout << "Test saving the model to YAML" << std::endl;
	std::string model_file_copy= "erfcopy_model.yml";
	std::string model_name_copy = "erf_model";
	erf_copy->save_model(model_file_copy, model_name_copy);
	std::cout << "Model saved" << std::endl;
	...
	for(int ex = 0; ex < train_samples.rows; ex++)
	{
		int prediction = erf_copy->predict(train_samples.row(ex));
		//std::cout << "+++ " << prediction << std::endl;
		if(prediction == train_labels.at<int>(ex))
			good_classif += 1;
	}
	...
	for(int ex = 0; ex < test_samples.rows; ex++)
	{
		int prediction = erf_copy->predict(test_samples.row(ex));
		//std::cout << "+++ " << prediction << std::endl;
		//pred_per_tree = erf->predict_with_idx(test_samples.row(ex));
		//std::cout << pred_per_tree << std::endl;
		if(prediction == test_labels.at<int>(ex))
