...
//*********************** PRIVATE FUNCTIONS *************************//
void NCMF_forest::format_training_data(std::string training_sets, cv::Mat& labels){	
	...
	while(parser.getline(buf,ssize,delimiter)){
		...
		training_samples.push_back(tmp_data);
		tmp_data.release();			

	}
	
	no_features = (int)training_samples.size();
	
}

void NCMF_forest::format_test_data(std::string test_sets, std::vector<cv::Mat>& test_samples){	
...
//*********************** PUBLIC FUNCTIONS *************************//
int NCMF_forest::get_no_features(){ return no_features;}

//training algorithm
void NCMF_forest::train(int depth_thresh, unsigned int samples_thresh, int classes_per_node, int no_trees, std::string training_sets)
{
	...
	///extract and format data from the specified file
	cv::Mat labels, training_data;
	format_training_data(training_sets, labels);
	
	//>[DEBUGGING]
	//std::cout << labels << std::endl;
	//for(size_t i = 0; i < training_samples.size(); i++){
		//cv::Mat tmp = training_samples[i];
	...
	for(int tree = 0; tree < max_trees; tree++)
	{
		...
		dectree->train(&training_samples, labels, depth_thresh, samples_thresh, classes_per_node);
...
//the last row only contains the label of the predicted class in the first element
...
void NCMF_forest::load_model(std::string filename, std::string name)
{
	...
	no_features = (int)training_params["noFeatures"];
