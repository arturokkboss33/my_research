...
int NCMF_class_tree::get_no_features() { return no_features;}
...
void NCMF_class_tree::train(const std::vector<cv::Mat>* ptr_training_data, const cv::Mat& labels, int depth_thresh, 
	unsigned int samples_thresh, int classes_per_node)
{
	...
	init_samples_vec = ptr_training_data;
	init_labels = labels.clone();
	no_features = (int)init_samples_vec->size();
...
void NCMF_class_tree::load_model(cv::FileNode& fnode_tree, cv::FileNode& fnode_params)
{
	...
	{	
		...
		(*node_it)["FeatureType"] >> nod->feature_type;
		(*node_it)["FeatureIdx"] >> nod->feature_idx;
	...
	const cv::Mat& samples_data, int depth, int no_split_fail)
{
	...
	if(samples_labels.empty())
	{
		dectree.insert_node(&dectree_root, "terminal", (++terminal_nodes_idx), depth, plurality(p_samples_labels), 
			dummy_table, dummy_table, samples_data, cv::Mat(), samples_labels, cv::Mat(), get_classes_hist(p_samples_labels),none, -1);
		return dectree_root;
	}
	...
	else if(check_classif(samples_labels))
	{
		dectree.insert_node(&dectree_root, "terminal", (++terminal_nodes_idx), depth, samples_labels.at<int>(0), 
			dummy_table, dummy_table, samples_data, cv::Mat(), samples_labels, cv::Mat(), get_classes_hist(samples_labels),none, -1);
		return dectree_root;
	}
...
NCMF_node* NCMF_class_tree::erf_split(const cv::Mat& samples, const cv::Mat& labels)
{
	...
	int selected_feature = (int)(rng.uniform(0., (double)no_features ));
	//std::cout << selected_feature << std::endl;
	cv::Mat init_samples = init_samples_vec->at(selected_feature);
	...
	*/
	...
	node->feature_idx = selected_feature;
